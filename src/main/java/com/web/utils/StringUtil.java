...
public class StringUtil {
	...
	public static String encode(String e_string, String method) {
		...
		try {
			if ("POST".equalsIgnoreCase(method))
				...
				r_string = URLEncoder.encode(
						URLEncoder.encode(e_string, "UTF-8"), "UTF-8");
	 ...
	 * @return String
	 */
	public static String processSplitedNumber(char[] num, int time) {
	 ...
	 * @return char[]
	 */
	public char[] revertCharArray(char[] array) {
	 ...
	 * @return 截掉之后返回的错误码
	 */
	public static String truncateString(String errCode, int length,
			boolean isRight) {
		if (!isEmpty(errCode)) {
			if (length >= errCode.length()) {
				...
				if (isRight) {
					...
					subString = errCode.substring(errCode.length() - length,
							errCode.length());
					return subString;
				}
			}
	...
	/**
	 * 隐藏手机号中间四位 $1、$2、……表示正则表达式里面第一个、第二个、……括号里面的匹配内容
	 * 
	 * @param mobile
	 * @return String
	 */
